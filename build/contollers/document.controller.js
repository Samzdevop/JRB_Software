...
exports.getDocumentTableOfContents = exports.getDocumentContent = exports.deleteDocument = exports.getDocumentStatus = exports.getDocuments = exports.getSearchHistory = exports.searchDocument = exports.uploadDocument = void 0;
...
const documentHelpers_1 = require("../utils/documentHelpers");
const fileExtractors_1 = require("../utils/fileExtractors");
...
const uploadDocument = async (req, res, next) => {
    try {
        ...
        if (!req.file) {
            return next(new BadRequestError_1.BadRequestError('No file uploaded'));
        }
        ...
        let extractedData;
        try {
            extractedData = await (0, fileExtractors_1.extractTextFromFile)(req.file.buffer, req.file.originalname, req.file.mimetype);
            ...
            return next(new BadRequestError_1.BadRequestError(`Failed to process file: ${errorMessage}`));
        }
        //Process and structure the text 
        const processedContent = textProcessor_1.TextProcessor.processRawText(extractedData.text, req.file.originalname);
        const document = await prisma_1.default.document.create({
            data: {
                title,
                filename: req.file.originalname,
                fileUrl: (0, multer_1.getFileUrl)(req.file.filename),
        ...
        processDocumentWithEnhancedAI(extractedData, document.id, req.file.mimetype)
            ...
            prisma_1.default.document.update({
                where: { id: document.id },
                data: { processingStatus: 'FAILED' },
            }).catch((error) => console.error('Failed to update document status:', error));
        });
        (0, sendSuccessResponse_1.sendSuccessResponse)(res, 'Document uploaded successfully. Processing with AI...', {
            document: {
                ...document,
...
const processDocumentWithEnhancedAI = async (extractedData, documentId, mimeType) => {
    try {
        ...
        const processedContent = textProcessor_1.TextProcessor.processRawText(extractedData.text, 'document' // or use the actual filename
        );
        ...
        await (0, documentHelpers_1.generateEmbeddingsForChunks)(documentId, openai);
        ...
        try {
            console.log('Falling back to basic text chunking');
            const basicChunks = await (0, documentHelpers_1.createChunksFromText)(extractedData.text, documentId);
            await prisma_1.default.document.update({
                where: { id: documentId },
                data: {
                    processed: true,
...
const shouldUseAIProcessing = (processedContent) => {
    // Use AI for complex documents with many sections
    if (processedContent.metadata.sectionCount > 20)
        return true;
    ...
    if (headingSections.length < 3)
        return true; // Not enough headings detected
    // Use AI for very long documents
    if (processedContent.metadata.wordCount > 10000)
        return true;
    return false;
};
const processWithAI = async (text, documentId, mimeType) => {
    const structuredContent = await openai.chat.completions.create({
        model: "gpt-3.5-turbo",
        messages: [
            {
    ...
    try {
        ...
        const processedContent = textProcessor_1.TextProcessor.processRawText(text, 'document');
        chunks = await createChunksFromStructuredContent(processedContent.structuredContent, documentId);
    }
    return chunks;
};
// Enhanced chunk creation from structured content (local processing)
const createChunksFromStructuredContent = async (structuredContent, documentId) => {
    const chunks = [];
    ...
    for (const section of structuredContent) {
        if (section.type === 'heading' && (section.level || 1) <= 2) {
            ...
            const formattedContent = textProcessor_1.TextProcessor.formatForDisplay([section]);
...
const createChunksFromAI = async (aiData, documentId, fullText) => {
    const chunks = [];
    // Handle different possible AI response formats
    if (aiData.chapters && Array.isArray(aiData.chapters)) {
        for (const chapter of aiData.chapters) {
    ...
    if (chunks.length === 0) {
        ...
        const processedContent = textProcessor_1.TextProcessor.processRawText(fullText, 'document');
...
const searchDocument = async (req, res, next) => {
    try {
        ...
        const userId = req.user.id;
        if (!query) {
            throw new BadRequestError_1.BadRequestError('Search query is required');
        }
        ...
        if (!document.processed) {
            throw new BadRequestError_1.BadRequestError('Document is still being processed. Please try again later.');
        }
        ...
        const searchHistory = await prisma_1.default.searchHistory.create({
            data: {
                query,
                results: {
                    chunks: finalChunks,
        ...
        (0, sendSuccessResponse_1.sendSuccessResponse)(res, 'Search completed', {
            results: {
                chunks: finalChunks,
                answer: aiResponse
            },
...
const generateContextualResponse = async (query, chunks, documentTitle) => {
    try {
        // Prepare the context from retrieved chunks
        const context = chunks.map(chunk => `[From ${chunk.chapter}, Page ${chunk.pageNumber}]: ${chunk.content.substring(0, 1000)}`).join('\n\n');
        ...
        if ((0, documentHelpers_1.shouldRejectAnswer)(answer, context)) {
...
const semanticSearch = async (query, documentId) => {
    try {
        ...
        const chunksWithSimilarity = chunks.map((chunk) => {
            ...
            const similarity = (0, documentHelpers_1.cosineSimilarity)(queryEmbedding, chunkEmbedding);
...
const keywordSearch = async (query, documentId) => {
    const queryKeywords = query.toLowerCase().split(/\s+/)
        .filter(k => k.length > 3) // Longer keywords are more meaningful
        .filter(k => !(0, documentHelpers_1.isCommonWord)(k));
...
const getSearchHistory = async (req, res, next) => {
    try {
        ...
        const searches = await prisma_1.default.searchHistory.findMany({
            where,
            include: {
                document: {
        ...
        const total = await prisma_1.default.searchHistory.count({ where });
        (0, sendSuccessResponse_1.sendSuccessResponse)(res, 'Search history retrieved', {
            searches,
            pagination: {
                page: Number(page),
                limit: Number(limit),
...
const getDocuments = async (req, res, next) => {
    try {
        ...
        const total = await prisma_1.default.document.count({ where: { uploadedById: userId } });
        (0, sendSuccessResponse_1.sendSuccessResponse)(res, 'Documents retrieved', {
            documents,
            pagination: {
                page: Number(page),
                limit: Number(limit),
...
const getDocumentStatus = async (req, res, next) => {
    try {
        ...
        (0, sendSuccessResponse_1.sendSuccessResponse)(res, 'Document status retrieved', { document });
    }
    catch (error) {
        next(error);
    }
...
const deleteDocument = async (req, res, next) => {
    try {
        ...
        (0, sendSuccessResponse_1.sendSuccessResponse)(res, 'Document and all related data deleted successfully');
    }
    catch (error) {
...
const getDocumentContent = async (req, res, next) => {
    try {
        ...
        if (!document.content) {
            throw new BadRequestError_1.BadRequestError('Document content not available');
        }
        let responseData = {};
        if (format === 'raw') {
            responseData = {
            ...
            const processedContent = textProcessor_1.TextProcessor.processRawText(document.content, document.filename);
            responseData = {
                content: textProcessor_1.TextProcessor.formatForDisplay(processedContent.structuredContent),
                format: 'formatted'
            };
        }
        else {
            ...
            responseData = {
                ...
                metadata: processedContent.metadata,
                tableOfContents: textProcessor_1.TextProcessor.generateTableOfContents(processedContent.structuredContent),
                format: 'structured'
            };
        }
        (0, sendSuccessResponse_1.sendSuccessResponse)(res, 'Document content retrieved', responseData);
    }
    catch (error) {
        next(error);
    }
...
const getDocumentTableOfContents = async (req, res, next) => {
    try {
        const { documentId } = req.params;
        const document = await prisma_1.default.document.findUnique({
            where: { id: documentId },
        ...
        const processedContent = textProcessor_1.TextProcessor.processRawText(document.content, document.filename);
        const tableOfContents = textProcessor_1.TextProcessor.generateTableOfContents(processedContent.structuredContent);
        (0, sendSuccessResponse_1.sendSuccessResponse)(res, 'Table of contents retrieved', {
            tableOfContents,
            metadata: processedContent.metadata
        });
    }
...
exports.getDocumentTableOfContents = getDocumentTableOfContents;
